...
const Hero = () => {
  ...
  return (
    ...
    >
      {/* Background Glow - DIUBAH KE HIJAU */}
      <div className={`absolute z-0 w-full h-[999px] overflow-hidden [@media(0px<width<=768px)]:h-[600px] [@media(768px<width<=1024px)]:h-[700px]`}>
        <div className={`absolute right-[0px] bottom-[0px] bg-[rgba(47,128,79,1)] w-[545.62px] h-[760.81px] rotate-[55.63deg] rounded-[50%] blur-[180px] [@media(0px<width<=768px)]:w-[300px] [@media(0px<width<=768px)]:h-[400px] [@media(768px<width<=1024px)]:w-[200px] [@media(768px<width<=1024px)]:h-[350px]`}></div>
      </div>
      ...
      <div className={`flex flex-col justify-center items-center h-full relative z-1 py-[80px] [@media(0px<width<=768px)]:py-[120px] [@media(768px<width<=1024px)]:py-[140px]`}>
        ...
        {/* Tombol ke Catalog - WARNA DIUBAH KE HIJAU */}
        <button
          onClick={() => navigate("/catalog")}
          className={`mt-12 bg-[rgba(47,128,79,1)] text-white font-plex font-bold text-[18px] px-12 py-4 rounded-full shadow-lg transition-all duration-300 hover:scale-105 hover:bg-[rgba(30,85,52,1)] active:scale-95 [@media(0px<width<=768px)]:text-[15px] [@media(0px<width<=768px)]:px-8 [@media(0px<width<=768px)]:mt-8`}
        >
...
const IdentitasPortal = () => {
  const itemSedia = [
    { 
      ...
      icon: (
        <svg viewBox="0 0 24 24" fill="none" stroke="#2F804F" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-[50%] h-[50%]">
    ...
    >
      ...
      <div className={`bg-[rgba(0,1,3,1)] w-[55%] h-auto rounded-[30px] pl-[100px] pr-[50px] pb-[80px]
        ...
        <h1 className={`text-[56px] text-white font-plex font-semibold mt-[118px] 
          ...
          <span className="bg-[linear-gradient(90deg,rgba(47,128,79,1),rgba(76,175,115,1),rgba(81,80,82,1))] bg-clip-text text-transparent">Platform Independen</span> yang Berpihak pada Mahasiswa
        </h1>
        ...
        <div className="bg-[linear-gradient(90deg,rgba(47,128,79,1),rgba(255,255,255,0))] w-[300px] h-[5px] mt-[30px] [@media(0px<width<=768px)]:w-[200px]"></div>
        ...
        <div className={`flex justify-center items-center bg-[rgba(51,49,56,1)] text-white font-plex h-[150px] mt-[70px] mr-[40px] border border-[rgba(47,128,79,1)] text-center px-6 
      ...
      <div className={`flex flex-col justify-center w-[45%] pr-[100px] py-[80px]
        ...
        <div className={`flex flex-col gap-[62px] w-full 
          ...
          {itemSedia.map((item) => (
            ...
            >
              <div className={`bg-[rgba(81,80,82,1)] border border-[rgba(47,128,79,1)] rounded-[10px] w-[93px] h-[93px] flex-shrink-0 flex items-center justify-center 
...
const LayananBeasiswa = () => {
  const itemBeasiswa = [
    { 
      ...
      icon: (
        <svg viewBox="0 0 24 24" fill="none" stroke="#2F804F" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="w-[40%] h-[40%]">
    ...
    >
      <div className={`flex flex-col mt-[136px] mx-[100px] gap-[30px] [@media(768px<width<=1024px)]:mt-[60px] [@media(768px<width<=1024px)]:mx-[40px] [@media(0px<width<=768px)]:mt-[40px] [@media(0px<width<=768px)]:mx-[24px]`}>
        {itemBeasiswa.map((item) => (
          <div
            key={item.jenis}
            className={`bg-[linear-gradient(90deg,rgba(47,128,79,0)40%,rgba(47,128,79,1)100%)] w-full h-[250px] [@media(0px<width<=768px)]:h-auto [@media(768px<width<=1024px)]:h-auto`}
          >
              ...
              <div className={`px-[20px] ml-[70px] w-[calc(100%-250px)] [@media(0px<width<=768px)]:ml-[0px] [@media(0px<width<=768px)]:w-full [@media(0px<width<=768px)]:px-[16px] [@media(0px<width<=768px)]:pb-[16px] [@media(768px<width<=1024px)]:ml-[24px] [@media(768px<width<=1024px)]:w-auto [@media(768px<width<=1024px)]:py-[16px]`}>
                <div className={`flex justify-between mt-[20px]`}>
                  <div className={`flex items-center`}>
                    <h2 className={`text-[rgba(47,128,79,1)] text-[24px] font-semibold [@media(0px<width<=768px)]:text-[18px] [@media(768px<width<=1024px)]:text-[20px]`}>{item.jenis}</h2>
                  </div>
                  <div className={`flex items-center`}>
                    <div className={`bg-[rgba(47,128,79,1)] text-[rgba(255,255,250,1)] text-[18px] flex items-center justify-center font-semibold font-plex w-[155px] h-[33px] rounded-[20px] [@media(0px<width<=768px)]:text-[13px] [@media(0px<width<=768px)]:w-[110px] [@media(0px<width<=768px)]:h-[28px]`}>
                      {item.pengelola}
                    </div>
                  </div>
                </div>
...
// ── Pengajuan Beasiswa ───────────────────────────────────
const listItems = [
  { text: "Prestasi Akademik", icon: <svg className="w-8 h-8" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 10v6M2 10l10-5 10 5-10 5z"/><path d="M6 12v5c3 3 9 3 12 0v-5"/></svg> },
  { text: "Motivasi Kuat", icon: <svg className="w-8 h-8" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg> },
  { text: "Keterbatasan Finansial", icon: <svg className="w-8 h-8" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="5" width="20" height="14" rx="2"/><path d="M12 12h.01M16 12h.01M8 12h.01"/></svg> },
  { text: "Visi ke Depan", icon: <svg className="w-8 h-8" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><path d="M12 2a7 7 0 1 0 10 10"/></svg> },
];
...
const PengajuanBeasiswa = () => {
  return (
    <div
      id="pengajuan"
      className="relative min-h-screen bg-[rgba(232,245,238,1)] overflow-hidden flex items-center py-20 [@media(0px<width<=1024px)]:py-10"
    >
      <div className="relative z-10 mx-[100px] w-full [@media(0px<width<=1024px)]:mx-[40px] [@media(0px<width<=768px)]:mx-[24px]">
        ...
        <h1 className="font-['IBM_Plex_Mono'] text-[48px] font-bold text-[rgba(0,0,0,1)] [@media(0px<width<=1024px)]:text-[36px] [@media(0px<width<=768px)]:text-[28px] [@media(0px<width<=768px)]:text-center">
          ...
          <span className="block text-[rgba(47,128,79,1)] font-semibold">mendapatkan beasiswa</span>
        </h1>
        ...
        <div className="flex flex-row justify-between items-stretch mt-[80px] gap-8 [@media(0px<width<=1024px)]:flex-col [@media(0px<width<=768px)]:mt-[40px]">
          ...
          <div className="flex flex-col gap-4 flex-[1.2]">
            {listItems.map((item) => (
              <div
                key={item.text}
                className="group flex items-center bg-[rgba(51,49,56,1)] text-[32px] text-[rgba(255,255,250,1)] font-plex font-semibold h-[120px] transition-all duration-300 hover:bg-[rgba(47,128,79,1)] hover:translate-x-4 [@media(0px<width<=1440px)]:text-[24px] [@media(0px<width<=1440px)]:h-[100px] [@media(0px<width<=768px)]:text-[18px]"
              >
                <div className="ml-[40px] text-[rgba(47,128,79,1)] group-hover:text-white transition-colors">
                  {item.icon}
                </div>
                <p className="ml-[40px]">{item.text}</p>
              </div>
            ))}
          </div>

          <div className="group border-2 border-[rgba(47,128,79,1)] rounded-[4px] p-8 bg-[rgba(252,234,232,0.6)] backdrop-blur-sm flex flex-col items-center justify-center gap-8 flex-1 transition-all duration-500 hover:shadow-[15px_15px_0px_0px_rgba(47,128,79,1)]">
            <div className="w-20 h-20 bg-[rgba(47,128,79,1)] rounded-full flex items-center justify-center text-white mb-4 animate-bounce">
              <svg className="w-10 h-10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>
              </svg>
            </div>
            <p className="font-['IBM_Plex_Mono'] text-[24px] text-center font-bold">"Tujuan studi yang jelas adalah kunci utama lolos seleksi."</p>
            <div className="h-1 w-20 bg-[rgba(47,128,79,1)]"></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="font-['IBM_Plex_Mono'] bg-[rgba(0,1,3,1)]">
      <section className="relative overflow-hidden mx-[100px] pt-20 pb-[72px] [@media(0px<width<=768px)]:mx-[24px]">
        <div className="relative z-10 max-w-[500px]">
          <h2 className="text-white text-[40px] font-bold leading-[1.2] mb-6">
            Siap Menemukan Beasiswamu?
          </h2>
          <p className="text-white/60 text-[14px] leading-[1.75] mb-10">
            Ratusan peluang beasiswa menunggumu. Mulai pencarian sekarang.
          </p>
          <div className="flex gap-5 flex-wrap">
            <button
              onClick={() => navigate("/catalog")}
              className="bg-[rgba(47,128,79,1)] text-white text-[15px] font-semibold px-12 py-[18px] rounded-[4px] transition-all duration-200 hover:opacity-85 hover:-translate-y-0.5"
            >
              Mulai Cari
            </button>
            <a href="#" className="bg-transparent text-white text-[15px] border border-white/35 px-12 py-[18px] rounded-[4px] hover:border-white">
      ...
      <div className="bg-[#140606] px-16">
        <hr className="border-none border-t border-[rgba(47,128,79,1)] opacity-70" style={{ borderTopWidth: "1px", borderTopStyle: "solid" }} />
      </div>
      <footer className="bg-[#140606] relative px-16 py-6 flex items-center justify-between [@media(0px<width<=768px)]:flex-col">
        ...
        <nav className="flex gap-8">
          {navLinks.map((link) => (
            <a key={link} href="#" className="text-white/50 text-[13px] hover:text-white transition-colors">
              {link}
            </a>
          ))}
        </nav>
